...
class QWatchConnector(BaseConnector):
    ...
    def _qwatch_query(self, search_term, search_type, query_type, search_args):
        ...
        try:
            return search_qwatch(search_term, search_type, query_type, **kwargs)
        except Exception as e:
            self.debug_print(f"qwatch lookup failed: {e!s}")
    ...
    def _make_qwatch_times(self, params):
        ...
        if QWATCH_START:
            ...
            self.debug_print("_make_qwatch_times", f"overriding start_time to {start_time}")

        # initial poll - start time
        if not start_time:
            if not (1 <= self.qwatch_initial_window <= 90):
                raise Exception("Config Error: QWatch Initial Ingestion Window parameter must be between 1 and 90")
    ...
    def _make_qwatch_params(self, param):
        ...
        req_params = {"date[start]": start_time, "date[end]": end_time, "meta[total]": True, "stats": True}
        ...
        if param.get("artifact_count"):
            ...
            if limit > 10000:
                self.save_progress("Adjusting artifact limit to maximum of 10000")
                limit = 1000
    ...
    def _process_exposures_ingest(self, data):
        ...
        container = {"name": "Qintel QWatch Alert", "description": f"{total_creds} exposed credentials", "artifacts": []}
    ...
    def _handle_qwatch_search(self, param):
        ...
        search_args = {"params": {"meta[total]": True, "stats": True}}

        try:
            results = self._qwatch_query(search_term, search_type, "exposures", search_args)
        except Exception as e:
        ...
        try:
            ...
            return action_result.set_status(phantom.APP_ERROR, f"Error occurred while processing the response from server {e!s}")
    ...
    def _handle_qwatch_poll(self, param):
        ...
        try:
            search_args = {"params": self._make_qwatch_params(param)}
    ...
    def initialize(self):
        ...
        ret_val, self.qwatch_initial_window = self._validate_integer(config.get("qwatch_initial_window", 6), "qwatch_initial_window")
